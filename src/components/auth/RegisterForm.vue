    ...
    <div class="w-full max-w-sm relative z-10">
      ...
      <div class="bg-white/5 backdrop-blur-xl rounded-xl py-8 px-7 transition-all duration-300 hover:bg-white/10 relative">
        ...
        <div v-if="isLoading" class="absolute inset-0 bg-black/70 backdrop-blur-md rounded-xl flex flex-col items-center justify-center gap-5 z-50">
          ...
          <p class="text-white/90 text-lg font-medium">Creating your account...</p>
        </div>

        <!-- Header - 100% same as Login -->
        <div class="text-center mb-8">
          <h1 class="text-2xl font-bold text-white mb-1 pacifico-regular">LinguaFlow</h1>
          <p class="text-teal-200/80 text-sm">Join thousands of translators worldwide</p>
        </div>
        ...
        <form @submit.prevent="handleSignup" class="space-y-4">
          ...
          <div>
            ...
            <input v-model="email" type="email" placeholder="you@example.com"
              class="w-full px-4 py-2.5 bg-white/10 rounded-lg text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:bg-white/15 transition-all duration-200" required />
          </div>
          ...
          <div>
            ...
            <div class="relative">
              <input v-model="password" :type="showPassword ? 'text' : 'password'" placeholder="Create a strong password"
                class="w-full px-4 py-2.5 bg-white/10 rounded-lg text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:bg-white/15 transition-all duration-200" required />
              <button type="button" @click="showPassword = !showPassword"
                class="absolute right-3 top-2.5 text-white/50 hover:text-white/80 transition">
          ...
          <!-- Confirm Password -->
          <div>
            <label class="block text-sm font-semibold text-white/90 mb-2">Confirm Password</label>
            <div class="relative">
              <input v-model="confirmPassword" :type="showConfirmPassword ? 'text' : 'password'" placeholder="Retype your password"
                class="w-full px-4 py-2.5 bg-white/10 rounded-lg text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:bg-white/15 transition-all duration-200" required />
    ...
    <Transition name="toast">
      <div v-if="toastMessage" class="fixed top-6 right-4 z-50 max-w-xs">
        <div :class="[
          ...
          toastMessage.includes('successfully') ? 'bg-emerald-500/20 text-emerald-100 border-emerald-500/50' : 'bg-red-500/20 text-red-100 border-red-500/50'
        ]">
          <span v-if="toastMessage.includes('successfully')" class="text-lg">Checkmark</span>
          {{ toastMessage }}
        </div>
      </div>
    </Transition>
...
const handleSignup = () => {
  ...
  if (!email.value || !password.value || !confirmPassword.value) {
    return toastMessage.value = 'Please fill all fields'
  }
  if (!email.value.includes('@')) {
    return toastMessage.value = 'Please enter a valid email'
  }
  if (password.value.length < 6) {
    return toastMessage.value = 'Password must be 6+ characters'
  }
...
const EyeOffIcon = () => h('svg', { width: "18", height: "18", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", 'stroke-width': "2", 'stroke-linecap': "round", 'stroke-linejoin': "round" }, [
